...
void aula() {
  ...
  aula = fopen(filename, "r+");

    while (exit) {
    fscanf(aula,"%d ", &aulanull.numero_ordem);
    if (aulanull.numero_ordem == -1) break;
    fscanf(aula, "%d/%d/%d %d %s ", &aulanull.data.dia, &aulanull.data.mes, &aulanull.data.ano, &aulanull.qntd_horas_ass, aulanull.conteudo);
    fprintf(aula," %d/%d/%d %d %s", aulanull.data.dia, aulanull.data.mes, aulanull.data.ano, aulanull.qntd_horas_ass, aulanull.conteudo);
...
void e() {
  int buscar,j=0;
  char buscar2[20],filename[20],testacabou[20],filename2[20];
  Taluno aluno[50];
  disciplina discs;
  FILE *fpdisc;
  ...
  do {
    fscanf(fpdisc,"%s %d ",buscar2,&buscar);
    sprintf(filename,"./alunos%d.txt",buscar2);
    sprintf(filename2,"./disciplina%d.txt",buscar2);
    FILE *alunos = fopen(filename,"r"), *disc = fopen(filename2,"r");

    fscanf(disc, "%s %s %d %c %d %d %d %d %d %f %f", discs.nome_disc, &discs.cod_disc, &discs.qntd_aluno, &discs.sem_ano.opcao,&discs.sem_ano.semestre_ano, &discs.qntd_provas, &discs.qntd_trabalhos,&discs.carga_horaria, &discs.carga_realizada, &discs.nota_min_aprov,&discs.percentual_freq);
    printf("%s %s %f %f",discs.nome_disc, discs.cod_disc,discs.nota_min_aprov,discs.percentual_freq);
    //do {
    //   fscanf(alunos, "%s", testacabou);
    //   if (strcmp(testacabou, "acabou") != 0) {
    //     strcpy(aluno[j].nome_aluno, testacabou);
